...
func main() {
	...
	fmt.Println("5. Testing GStreamer pipeline with FD and path as string...")
	cmd := exec.Command("gst-launch-1.0", "pipewiresrc", "fd=3", fmt.Sprintf("path=\"%d\"", nodeID), "!", "videoconvert", "!", "video/x-raw,format=I420", "!", "fakesink", "num-buffers=30")
	cmd.ExtraFiles = append(cmd.ExtraFiles, os.NewFile(uintptr(pipewireFd), "pipewire-fd"))
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err = cmd.Run()
	if err != nil {
		fmt.Printf("GStreamer failed: %v\n", err)
	} else {
		fmt.Println("GStreamer SUCCESS!")
	}
}
