...
// 100.x IPs when tsnet is in userspace mode (no kernel tun interface on Android).
//
// Architecture:
//
...
func (p *moonlightTSNetProxy) handleRTSP(client net.Conn) {
	...
	go func() {
		...
		// client→server: pass through (no inspection needed anymore)
		copyConn(server, client)
	}()
	go func() {
		defer func() { done <- struct{}{} }()
		// server→client: inspect SETUP responses, rewrite server_port → local UDP proxy
...
func (p *moonlightTSNetProxy) rewriteServerPorts(data []byte) []byte {
	...
	for {
		...
		if !seen {
			...
			// Reset offset after rewrite (port digit count may differ).
