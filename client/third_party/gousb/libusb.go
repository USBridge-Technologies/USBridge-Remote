...
type libusbIntf interface {
	...
	clearHalt(*libusbDevHandle, uint8) error
...
// clearHalt clears a halt/stall condition on ep (full address, direction bit
// included) using the SAME handle libusb already claimed the interface on —
// unlike a fresh independent usbfs open of the device node, this carries the
// kernel-side "this fd owns the claimed interface" bookkeeping that
// USBDEVFS_CLEAR_HALT requires, so it doesn't fail with EBUSY / "did not
// claim interface before use".
func (libusbImpl) clearHalt(d *libusbDevHandle, ep uint8) error {
	return fromErrNo(C.libusb_clear_halt((*C.libusb_device_handle)(d), C.uchar(ep)))
}
