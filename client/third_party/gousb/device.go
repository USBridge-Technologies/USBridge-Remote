...
// ClearHalt clears a halt/stall condition on ep (the full endpoint address,
// with the direction bit set, e.g. 0x81 for EP1 IN) on this device's own
// handle. Use this — not a fresh open of the device node — to recover from a
// bulk transfer that failed with a STALL: the endpoint's halt is tracked by
// the host controller as well as the device, and clearing it needs to go
// through the same handle that claimed the owning interface.
func (d *Device) ClearHalt(ep uint8) error {
	if d.handle == nil {
		return fmt.Errorf("ClearHalt() called on %s after Close", d)
	}
	return d.ctx.libusb.clearHalt(d.handle, ep)
}
