...
// request: both checkboxes together must request
// VIDEO_FORMAT_H265_REXT10_444 (0x0800, 4:4:4 AND 10-bit) -- no backend
// implements that combination today (see moonlightVideoFormat's own doc
// comment), but the request itself is still well-defined and independently
// testable regardless of what any particular server can satisfy.
func TestMoonlightVideoFormatColor444AndHdrCombine(t *testing.T) {
	if got := moonlightVideoFormat(models.VideoModeH265, true, true); got != 0x0800 {
		t.Errorf("moonlightVideoFormat(h265, color444=true, hdr=true) = 0x%04X, want VIDEO_FORMAT_H265_REXT10_444 (0x0800)", got)
	}
	if codec, ok := videoFormatCodecName(0x0800); !ok || codec != models.VideoModeH265 {
		t.Errorf("videoFormatCodecName(0x0800) = (%q, %v), want (%q, true)", codec, ok, models.VideoModeH265)
	}
}
