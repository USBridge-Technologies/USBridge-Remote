...
// NOT called from the render thread to avoid Go-GC deadlock.
...
// GLVideoTrySubmit offers a BGRA frame to the GDI render thread.
// Returns true if the overlay is active and the frame was queued.
...
// GLVideoCreate creates (or replaces) the GDI child-window overlay on the given HWND.
...
// Last overlay geometry sent to C — suppresses no-op SetWindowPos calls.
...
// GLVideoUpdateFrame repositions the GDI child overlay (pixels).
// Skips if geometry hasn't changed.
...
// GLVideoResetLastFrame resets cached geometry so the next GLVideoUpdateFrame
// unconditionally repositions (call before GLVideoCreate).
