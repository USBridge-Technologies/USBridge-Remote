...
// "download finishes, then nothing visibly happens" bug: a running
// AppImage's os.Executable() resolves through /proc/self/exe to its own
// read-only FUSE squashfs mount under /tmp/.mount_<random>/, so
// installTargetPath must prefer $APPIMAGE (the real, writable .AppImage
// file) whenever it's set.
func TestInstallTargetPathPrefersAPPIMAGE(t *testing.T) {
	dir := t.TempDir()
	real := filepath.Join(dir, "USBridgeClient-Linux-x86_64-9.9.9.AppImage")
...
func TestStageInDirSameFilesystemRename(t *testing.T) {
	...
	final := filepath.Join(dir, "USBridgeClient.AppImage")
