...
const (
	accountLicenseIDTextSize = float32(8)
	accountLicenseIDMaxLines = 3
	accountLicenseIDMinWrapW = float32(80)
	accountLicenseClusterGap = float32(6)
	accountLicenseCopySlot   = float32(24)
	accountLicenseCopyGap    = float32(8)
)

func accountLicenseLineHeight() float32 {
	return fyne.MeasureText("Ag", accountLicenseIDTextSize, fyne.TextStyle{}).Height
}

// accountLicenseInnerBudget is the width left for kind + identifier + status
// inside the desktop Account dialog at its fixed 420px panel width (scroll
// inset 21×2, card inset 14×2, copy button + gap). Used only to reserve
// wrap height; Layout uses the real allocated width.
func accountLicenseInnerBudget() float32 {
	return 420 - 21*2 - 14*2 - accountLicenseCopySlot - accountLicenseCopyGap
}
...
func newAccountLicenseRow(kind, identifier, status string, window fyne.Window) fyne.CanvasObject {
	...
	kindText.TextSize = accountLicenseIDTextSize
	kindText.TextStyle = fyne.TextStyle{Bold: true}

	idText := newAccountLicenseID(identifier)

	statusColor := licenseStatusColor(status)
	statusText := canvas.NewText("— "+status, statusColor)
	statusText.TextSize = accountLicenseIDTextSize

	left := container.New(&accountLicenseTextLayout{identifier: identifier}, kindText, idText, statusText)
...
// accountLicenseTextLayout places "[kind] identifier — status" so a long
// hex identifier wraps (up to 3 lines) instead of stretching the dialog.
// MinSize.Width deliberately ignores the unwrapped identifier length.
type accountLicenseTextLayout struct {
	identifier string
}

func (l *accountLicenseTextLayout) MinSize(objects []fyne.CanvasObject) fyne.Size {
	if len(objects) < 3 {
		return fyne.NewSize(0, 24)
	}
	kindMin := objects[0].MinSize()
	statusMin := objects[2].MinSize()
	lineH := accountLicenseLineHeight()
	idW := fyne.MeasureText(l.identifier, accountLicenseIDTextSize, fyne.TextStyle{}).Width
	wrapW := accountLicenseInnerBudget() - kindMin.Width - statusMin.Width - accountLicenseClusterGap*2
	if wrapW < accountLicenseIDMinWrapW {
		wrapW = accountLicenseIDMinWrapW
	}
	lines := 1
	if idW > wrapW {
		lines = int(math.Ceil(float64(idW / wrapW)))
	}
	if lines > accountLicenseIDMaxLines {
		lines = accountLicenseIDMaxLines
	}
	h := lineH * float32(lines)
	if h < 24 {
		h = 24
	}
	reportedID := idW
	...
	return fyne.NewSize(kindMin.Width+accountLicenseClusterGap+reportedID+accountLicenseClusterGap+statusMin.Width, h)
}

func (l *accountLicenseTextLayout) Layout(objects []fyne.CanvasObject, size fyne.Size) {
	if len(objects) < 3 {
		return
	}
	kind, id, status := objects[0], objects[1], objects[2]
	kindMin := kind.MinSize()
	statusMin := status.MinSize()
	lineH := accountLicenseLineHeight()

	kind.Resize(kindMin)
	kind.Move(fyne.NewPos(0, 0))

	status.Resize(statusMin)
	statusX := size.Width - statusMin.Width
	if statusX < kindMin.Width+accountLicenseClusterGap {
		statusX = kindMin.Width + accountLicenseClusterGap
	}
	status.Move(fyne.NewPos(statusX, 0))

	idX := kindMin.Width + accountLicenseClusterGap
	idW := statusX - accountLicenseClusterGap - idX
	if idW < accountLicenseIDMinWrapW {
		idW = accountLicenseIDMinWrapW
	}
	idH := size.Height
	if minH := lineH; idH < minH {
		idH = minH
	}
	id.Resize(fyne.NewSize(idW, idH))
	id.Move(fyne.NewPos(idX, 0))
}

// accountLicenseID draws a license identifier with character wrap (hex ids
// have no spaces, so word-wrap would still be one giant line) and a hard
// cap of accountLicenseIDMaxLines, with an ellipsis if it still overflows.
type accountLicenseID struct {
	widget.BaseWidget
	text string
}

func newAccountLicenseID(identifier string) *accountLicenseID {
	w := &accountLicenseID{text: identifier}
	w.ExtendBaseWidget(w)
	return w
}

func (w *accountLicenseID) CreateRenderer() fyne.WidgetRenderer {
	lines := make([]*canvas.Text, accountLicenseIDMaxLines)
	for i := range lines {
		t := canvas.NewText("", design.ColorTextLight)
		t.TextSize = accountLicenseIDTextSize
		t.Hide()
		lines[i] = t
	}
	return &accountLicenseIDRenderer{id: w, lines: lines}
}

type accountLicenseIDRenderer struct {
	id    *accountLicenseID
	lines []*canvas.Text
}

func (r *accountLicenseIDRenderer) MinSize() fyne.Size {
	// Width 0: the parent layout allocates wrap width. Height is one line;
	// the parent reserves up to 3 lines from the identifier length.
	return fyne.NewSize(0, accountLicenseLineHeight())
}

func (r *accountLicenseIDRenderer) Layout(size fyne.Size) {
	lineH := accountLicenseLineHeight()
	maxLines := int(size.Height / lineH)
	if maxLines > accountLicenseIDMaxLines {
		maxLines = accountLicenseIDMaxLines
	}
	if maxLines < 1 {
		maxLines = 1
	}
	parts := wrapLicenseIdentifier(r.id.text, size.Width, maxLines)
	for i, t := range r.lines {
		if i < len(parts) {
			t.Text = parts[i]
			t.Show()
			t.Move(fyne.NewPos(0, float32(i)*lineH))
			t.Resize(fyne.NewSize(size.Width, lineH))
			t.Refresh()
		} else {
			t.Text = ""
			t.Hide()
...
func (r *accountLicenseIDRenderer) Objects() []fyne.CanvasObject {
...
func (r *accountLicenseIDRenderer) Refresh() {
	for _, t := range r.lines {
		t.Refresh()
	}
}

func (r *accountLicenseIDRenderer) Destroy() {}

func wrapLicenseIdentifier(s string, maxWidth float32, maxLines int) []string {
	if s == "" {
		return []string{""}
	}
	if maxLines < 1 {
	...
	if maxWidth <= 0 || fyne.MeasureText(s, accountLicenseIDTextSize, style).Width <= maxWidth {
		return []string{s}
	}
	runes := []rune(s)
	var lines []string
	start := 0
	for len(lines) < maxLines && start < len(runes) {
		rest := runes[start:]
		if fyne.MeasureText(string(rest), accountLicenseIDTextSize, style).Width <= maxWidth {
			lines = append(lines, string(rest))
			break
		}
		last := len(lines) == maxLines-1
		fit := licensePrefixFit(rest, style, maxWidth, last)
		if fit <= 0 {
			fit = 1
		}
		if last && start+fit < len(runes) {
			lines = append(lines, string(rest[:fit])+"…")
			break
		}
		lines = append(lines, string(rest[:fit]))
		start += fit
...
func licensePrefixFit(runes []rune, style fyne.TextStyle, maxWidth float32, ellipsis bool) int {
	suffix := ""
	if ellipsis {
		suffix = "…"
	}
	...
	for lo < hi {
		mid := (lo + hi + 1) / 2
		if fyne.MeasureText(string(runes[:mid])+suffix, accountLicenseIDTextSize, style).Width <= maxWidth {
			lo = mid
		} else {
			hi = mid - 1
		}
