...
func (dw *DiskWidget) groupDriveIndexes() []deviceSection {
	...
	for idx, drive := range dw.allDrives {
		...
		case drive.IsUSBPassthrough:
			passthrough = append(passthrough, idx)
	...
	return []deviceSection{
		...
		{
			key:         "passthrough",
			title:       i18n.Current.DevicesSectionPassthrough,
			description: i18n.Current.DevicesSectionPassthroughHint,
...
func driveBadge(drive DriveItem) (string, color.Color) {
	...
	case drive.IsUSBPassthrough:
		return "USB", color.NRGBA{R: 0x6b, G: 0x3a, B: 0x2c, A: 0xff}
	case drive.Source == "api" && drive.LocalDrive != nil && drive.LocalDrive.SourceType == "mtp":
