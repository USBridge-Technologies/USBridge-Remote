...
> ⚠️ **Beta 软件** — 这是一个早期版本。请预期会有错误。请通过 [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues) 报告问题，或加入我们的 [Discord](https://discord.com/invite/xqQ6ybkfWS) 寻求支持。
> 
...
| 媒体 | 亮点 | 链接 |
| :--- | :--- | :---: |
| **IlSoftware.it**（意大利） | *"USBridge Remote 挑战 RustDesk 和 AnyDesk..."* — 独立深度评测，赞扬软件代理与硬件 KVM 的协同作用、原生 Wayland 支持和 P2P 架构。 | [阅读文章](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**一切尽在一处** — 我已统一工作流程。从单一仪表板管理 USBridge KVM 硬件和软件代理。添加机器，连接，然后您就可以使用了。

**没有限制，没有订阅** — 完全免费。没有会话时间限制，没有连接上限，也不需要在目标机器上创建帐户。

**低延迟视频和 Moonlight 集成** — 享受高达 2K 分辨率的流畅 240 FPS 视频，几乎没有可感知的延迟。我的自适应流媒体引擎利用原生 Moonlight 集成，提供无与伦比的超低延迟远程桌面性能。
...
> **注意：** 选择不同的显示器后，您必须重新连接会话以重新初始化视频流并显示新屏幕。
...
大多数 Linux 上的远程桌面代理在使用 Wayland 时会遇到困难，或者每次会话开始时都会不断向您发送权限提示和确认弹窗。

我设计的 USBridge Agent 原生支持 Wayland。它可以开箱即用地处理全屏捕获和输入注入，**无需任何烦人的权限提示**或手动确认。它就是这样工作。
