...
**USBridge Remote** 是一个统一的高性能客户端，用于管理远程机器。我设计它是为了将 **硬件级 BIOS 访问**（通过 USBridge KVM 设备）和 **基于软件的远程桌面** 结合在一个简化的界面中。

 🖥️ **需要在操作系统启动之前进行硬件级 BIOS 控制吗？**  
 USBridge Remote 原生集成 **USBridge-KVM 2.0**，用于带外、金属级管理。
...
> ⚠️ **测试版软件** — 这是一个早期版本。请预期会有错误。请通过 [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues) 报告问题，或加入我们的 [Discord](https://discord.com/invite/xqQ6ybkfWS) 寻求支持。
> 
> ℹ️ **关于 Windows Defender / 杀毒软件误报的说明：**  
> Windows Defender 可能会错误地将 `libva.dll` 标记为威胁（`Trojan:Win32/Wacatac.B!ml`），这是由于对未签名二进制文件的启发式/机器学习检测。**这是假阳性。**  
...
| **ARM64** | — | [下载](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest/download/USBridgeClient-macOS-arm64.dmg) | — | [下载](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest/download/USBridgeClient-Android-arm64-selfupdate.apk) | [App Store](https://apps.apple.com/us/app/usbridge-client/id6787665935) | [打开应用](https://web.usbridge.io) |

🌐 **零安装网页客户端**：无需安装。只需打开 [web.usbridge.io](https://web.usbridge.io) 即可立即连接。*(注意：网页客户端由于浏览器安全沙箱和 WebRTC 限制，某些功能和性能有限制。要获得完整无妨碍的体验，请使用本地应用程序)。*

## 代理

代理运行在目标机器上 — 您希望远程访问的服务器或 PC。它处理屏幕捕获、输入注入和 Tailscale 网络。
...
| 媒体 | 精华 | 链接 |
| :--- | :--- | :---: |
| **IlSoftware.it**（意大利） | *"USBridge Remote 挑战 RustDesk 和 AnyDesk..."* — 独立深入评测，赞扬软件代理与硬件 KVM 的协同、原生 Wayland 支持和 P2P 架构。 | [阅读文章](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**一处搞定所有** — 我统一了工作流程。从单一仪表板管理 USBridge KVM 硬件和软件代理。添加一台机器，连接，即可使用。

**无限制，无需订阅** — 完全免费。没有会话时间限制，没有连接上限，目标机器上无需账户。

**低延迟视频与 Moonlight 集成** — 享受高达 2K 分辨率的流畅 240 FPS，且没有可感知的延迟。我的自适应流媒体引擎利用原生 Moonlight 集成，提供无与伦比的超低延迟远程桌面性能。

**Tailscale 集成** — 内置加密 P2P 隧道。无需处理端口转发或防火墙规则，即可全球连接任何机器。它在局域网和互联网自动工作。

**共享剪贴板** — 在本地机器和远程目标之间无缝复制和粘贴。它完全支持文本、图像和文件传输。

**多显示器支持** — 我增加了在多个显示器之间切换的能力。如果目标机器有多个显示器，您现在可以轻松选择要直接从连接设置中查看的显示器。
...
大多数 Linux 上的远程桌面代理在 Wayland 上表现不佳，或者每次会话开始时都会不断向您发送权限提示和确认弹窗。

我设计的 USBridge Agent 原生支持 Wayland。它可以开箱即用地处理全屏捕获和输入注入 **而无需任何烦人的权限提示** 或手动确认。它就是这么好用。
...
加入我们的 Discord 以获得 **Beta Tester** 角色，报告错误，并帮助我制定路线图：
