...
**USBridge Remote**는 원격 머신 관리를 위한 통합 고성능 클라이언트입니다. **하드웨어 수준 BIOS 접근** (USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크탑**을 단일의 간소화된 인터페이스로 결합하도록 설계하였습니다.

 🖥️ **OS 부팅 전에 하드웨어 수준 BIOS 제어가 필요하신가요?**  
 USBridge Remote는 **USBridge-KVM 2.0**과 네이티브로 통합되어 아웃 오브 밴드, 메탈 수준 관리를 제공합니다.
...
> ⚠️ **베타 소프트웨어** — 이는 초기 릴리스입니다. 버그가 발생할 수 있습니다. 문제를 보고하려면 [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues)를 통해 신고하시거나 지원을 위해 [Discord](https://discord.com/invite/xqQ6ybkfWS)에 참여해 주세요.
> 
> ℹ️ **Windows Defender / 바이러스 백신의 잘못된 경고에 대한 주의:**  
> Windows Defender가 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`) 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 감지로 인해. **이는 잘못된 경고입니다.**  
> 우리는 파일을 Microsoft Security Intelligence에 제출하여 공식 검토 및 화이트리스트 등록을 요청했습니다. 그동안 바이러스 백신이 `libva.dll`을 제거하면 격리에서 복원하거나 USBridge 폴더를 바이러스 백신 제외 목록에 추가해 주세요.
...
### 클라이언트
클라이언트는 제어 인터페이스입니다 — 워크스테이션이나 노트북에 설치되거나 브라우저에서 직접 실행됩니다. 연결, 실시간 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 관리합니다.
...
Play 스토어 계정 없이 직접 APK를 선호하시나요? 자동 업데이트 빌드도 [최신 릴리스](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest)에 게시됩니다.

🌐 **제로 설치 웹 클라이언트**: 설치가 필요 없습니다. [web.usbridge.io](https://web.usbridge.io)를 열어 즉시 연결하세요. *(참고: 웹 클라이언트는 브라우저 보안 샌드박스 및 WebRTC 제약으로 인해 일부 기능 및 성능 제한이 있습니다. 완전한 경험을 원하신다면 네이티브 앱을 사용하세요).*
...
**모든 것을 위한 하나의 장소** — 워크플로우를 통합했습니다. 단일 대시보드에서 USBridge KVM 하드웨어와 소프트웨어 에이전트를 관리하세요. 머신을 추가하고 연결하면 됩니다.

**제한 없음, 구독 없음** — 완전히 무료입니다. 세션 시간 제한, 연결 제한이 없으며, 대상 머신에서 계정이 필요하지 않습니다.

**저지연 비디오 및 Moonlight 통합** — 부드러운 240 FPS로 최대 2K 해상도를 즐기세요. 나의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비교할 수 없는 초저지연 원격 데스크탑 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결할 수 있습니다. LAN 및 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 복사 및 붙여넣기를 원활하게 수행하세요. 텍스트, 이미지 및 파일 전송을 완벽하게 지원합니다.

**다중 모니터 지원** — 여러 디스플레이 간 전환할 수 있는 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 이제 연결 설정에서 쉽게 보고 싶은 모니터를 선택할 수 있습니다.
...
대부분의 리눅스 원격 데스크탑 에이전트는 Wayland와 호환되지 않거나 세션이 시작될 때마다 권한 프롬프트와 확인 팝업을 지속적으로 표시합니다.

USBridge 에이전트는 Wayland를 네이티브로 지원하도록 설계되었습니다. **귀찮은 권한 프롬프트**나 수동 확인 없이 전체 화면 캡처 및 입력 주입을 처리합니다. 그냥 작동합니다.

---

## 빠른 시작

1. **에이전트 설치**: 원격으로 접근할 머신에 에이전트를 설치하세요. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근이 필요하면 Tailscale을 연결하세요.

2. **클라이언트 설치**: 워크스테이션, 노트북 또는 휴대폰에 클라이언트를 설치하세요.

3. **연결 추가**: 에이전트 창에 표시된 IP 또는 Tailscale 주소를 입력하세요. 그게 전부입니다.
...
베타 테스터 역할을 받고 버그를 보고하며 로드맵을 형성하는 데 도움을 주려면 Discord에 참여하세요:
