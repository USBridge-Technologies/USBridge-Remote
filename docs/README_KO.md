...
**USBridge Remote**는 원격 머신 관리를 위한 통합 고성능 클라이언트입니다. **하드웨어 수준의 BIOS 접근** (USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크탑**을 단일화된 인터페이스로 결합하도록 설계했습니다.
...
> 
> ℹ️ **Windows Defender / Antivirus 허위 긍정 사항에 대한 주의:**  
> Windows Defender는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지로 인해 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`). **이는 허위 긍정입니다.**  
> 우리는 Microsoft Security Intelligence에 파일을 제출하여 공식 검토 및 화이트리스트 등록을 요청했습니다. 그동안 귀하의 안티바이러스가 `libva.dll`을 제거하는 경우, 격리에서 복원하거나 USBridge 폴더를 안티바이러스 제외 목록에 추가해 주세요.
...
### 클라이언트
클라이언트는 제어 인터페이스로, 작업용 스테이션이나 노트북에 설치됩니다. 연결, 실시간 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 관리합니다.
...
에이전트는 원격으로 접근하고자 하는 대상 머신(서버 또는 PC)에서 실행됩니다. 화면 캡처, 입력 주입 및 Tailscale 네트워킹을 처리합니다.
...
| 미디어 | 하이라이트 | 링크 |
| :--- | :--- | :---: |
| **IlSoftware.it** (이탈리아) | *"USBridge Remote는 RustDesk와 AnyDesk에 도전합니다..."* — 소프트웨어 에이전트 및 하드웨어 KVM의 시너지, 네이티브 Wayland 지원 및 P2P 아키텍처를 칭찬하는 독립적인 심층 리뷰. | [기사 읽기](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**모든 것을 위한 하나의 장소** — 워크플로를 통합했습니다. 단일 대시보드에서 USBridge KVM 하드웨어 및 소프트웨어 에이전트를 관리하세요. 머신을 추가하고 연결하면 됩니다.
...
**다중 모니터 지원** — 여러 디스플레이 간 전환 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 이제 연결 설정에서 직접 어떤 모니터를 볼지 쉽게 선택할 수 있습니다.
...
대부분의 리눅스 원격 데스크탑 에이전트는 Wayland에서 어려움을 겪거나 세션이 시작될 때마다 권한 프롬프트 및 확인 팝업을 지속적으로 표시합니다.
...
1. **에이전트를 설치**합니다. 원격으로 접근하고자 하는 머신에서 실행하세요. 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근이 필요하다면 Tailscale을 연결하세요.

2. **클라이언트를 설치**합니다. 작업용 스테이션, 노트북 또는 전화에 설치하세요.
...
소프트웨어 개발 계획 및 향후 기능을 오픈 대시보드에서 관리합니다. 현재 개발 중인 사항, 계획된 사항 또는 향후 기능의 상태를 확인하고 싶다면 라이브 로드맵을 확인하세요:
