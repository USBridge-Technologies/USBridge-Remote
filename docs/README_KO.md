...
**USBridge Remote**는 원격 머신 관리를 위한 통합 고성능 클라이언트입니다. **하드웨어 수준의 BIOS 접근**(USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크탑**을 단일화된 인터페이스로 결합하도록 설계했습니다.
 ...
 🖥️ **OS 부팅 전 하드웨어 수준의 BIOS 제어가 필요하신가요?**  
 USBridge Remote는 **USBridge-KVM 2.0**과 원활하게 통합되어 아웃 오브 밴드, 메탈 수준 관리가 가능합니다.
...
> ⚠️ **베타 소프트웨어** — 이는 초기 릴리스입니다. 버그가 발생할 수 있습니다. 문제를 보고하려면 [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues)를 통해 또는 지원을 위해 [Discord](https://discord.com/invite/xqQ6ybkfWS)에 참여해 주세요.
> 
> ℹ️ **Windows Defender / Antivirus 허위 경고에 대한 주의:**  
> Windows Defender는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지로 인해 `libva.dll`을 위협으로 잘못 표시할 수 있습니다(`Trojan:Win32/Wacatac.B!ml`). **이는 허위 경고입니다.**  
...
### 클라이언트
클라이언트는 제어 인터페이스로, 작업용 컴퓨터나 노트북에 설치됩니다. 연결 관리, 실시간 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 처리합니다.
...
에이전트는 원격으로 접근하려는 대상 머신(서버 또는 PC)에서 실행됩니다. 화면 캡처, 입력 주입 및 Tailscale 네트워킹을 처리합니다.
...
**저지연 비디오 및 Moonlight 통합** — 부드러운 240 FPS로 최대 2K 해상도를 즐기며, 지각 가능한 지연이 없습니다. 저의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비할 데 없는 초저지연 원격 데스크탑 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결할 수 있습니다. LAN 및 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사하고 붙여넣기 할 수 있습니다. 텍스트, 이미지 및 파일 전송을 기본적으로 완벽하게 지원합니다.
...
대부분의 리눅스 원격 데스크탑 에이전트는 Wayland와의 호환성에 어려움을 겪거나 세션이 시작될 때마다 권한 프롬프트 및 확인 팝업으로 귀찮게 합니다.
...
1. **에이전트를 설치**합니다. 원격으로 접근하려는 머신에서 실행하세요. 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근해야 하는 경우 Tailscale을 연결하세요.

2. **클라이언트를 설치**합니다. 작업용 컴퓨터, 노트북 또는 휴대폰에 설치하세요.
...
소프트웨어 개발 계획 및 향후 기능을 공개 대시보드에서 관리합니다. 현재 개발 중인 사항, 계획된 사항 또는 향후 기능의 상태를 확인하려면 라이브 로드맵을 확인하세요:
