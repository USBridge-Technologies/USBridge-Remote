...
**USBridge Remote**는 원격 머신을 관리하기 위한 통합 고성능 클라이언트입니다. **하드웨어 수준의 BIOS 접근**(USBridge KVM 장치를 통해)과 **소프트웨어 기반의 원격 데스크탑**을 하나의 간소화된 인터페이스로 결합하도록 설계했습니다.
...
> Windows Defender가 `libva.dll`을 위협으로 잘못 표시할 수 있습니다(`Trojan:Win32/Wacatac.B!ml`) 이는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지로 인한 것입니다. **이는 허위 긍정입니다.**  
> 우리는 Microsoft Security Intelligence에 파일을 제출하여 공식 검토 및 화이트리스트 등록을 요청했습니다. 그동안 귀하의 안티바이러스가 `libva.dll`을 제거하면 격리에서 복원하거나 USBridge 폴더를 안티바이러스 제외 목록에 추가해 주세요.
...
| 미디어 | 하이라이트 | 링크 |
| :--- | :--- | :---: |
| **IlSoftware.it** (이탈리아) | *"USBridge Remote는 RustDesk와 AnyDesk에 도전합니다..."* — 소프트웨어 에이전트 & 하드웨어 KVM 시너지, 네이티브 Wayland 지원 및 P2P 아키텍처를 칭찬하는 독립적인 심층 리뷰. | [기사 읽기](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**모든 것을 위한 하나의 장소** — 워크플로를 통합했습니다. 단일 대시보드에서 USBridge KVM 하드웨어와 소프트웨어 에이전트를 관리하세요. 머신을 추가하고 연결하면 됩니다.

**제한 없음, 구독 없음** — 완전히 무료입니다. 세션 시간 제한, 연결 제한이 없으며 대상 머신에서 계정이 필요하지 않습니다.

**저지연 비디오 및 Moonlight 통합** — 부드러운 240 FPS와 눈에 띄는 지연 없이 최대 2K 해상도를 즐기세요. 저의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비할 데 없는 초저지연 원격 데스크탑 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결할 수 있습니다. LAN 및 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사 및 붙여넣기를 지원합니다. 텍스트, 이미지 및 파일 전송을 완벽하게 지원합니다.
...
USBridge 에이전트는 Wayland를 네이티브로 지원하도록 설계되었습니다. 전체 화면 캡처 및 입력 주입을 **귀찮은 권한 프롬프트나 수동 확인 없이** 즉시 처리합니다. 그냥 작동합니다.

---

## 빠른 시작

1. **에이전트 설치** — 원격으로 접근하려는 머신에 설치합니다. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근이 필요하면 Tailscale을 연결하세요.

2. **클라이언트 설치** — 워크스테이션, 노트북 또는 전화에 설치합니다.
