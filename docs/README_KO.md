...
**USBridge Remote**는 원격 머신 관리를 위한 통합 고성능 클라이언트입니다. **하드웨어 수준 BIOS 접근**(USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크톱**을 단일화된 인터페이스로 결합하도록 설계되었습니다.

 🖥️ **OS 부팅 전에 하드웨어 수준 BIOS 제어가 필요하신가요?**  
 USBridge Remote는 **USBridge-KVM 2.0**과 네이티브로 통합되어 아웃 오브 밴드, 메탈 수준 관리가 가능합니다.
...
> Windows Defender가 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`) 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지로 인해. **이는 잘못된 긍정입니다.**  
> 우리는 Microsoft 보안 인텔리전스에 파일을 공식 검토 및 화이트리스트 요청을 제출했습니다. 그동안 귀하의 안티바이러스가 `libva.dll`을 제거하면 격리에서 복원하거나 USBridge 폴더를 안티바이러스 제외 목록에 추가해 주세요.
...
### 클라이언트
클라이언트는 제어 인터페이스입니다 — 작업 공간이나 노트북에 설치되거나 브라우저에서 직접 실행됩니다. 연결, 라이브 원격 데스크톱, 가상 장치 패스스루 및 스냅샷 레지스트리를 관리합니다.
...
Play 스토어 계정 없이 직접 APK를 선호하십니까? 자동 업데이트 빌드도 [최신 릴리스](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest)에 게시됩니다.
...
**저지연 비디오 및 Moonlight 통합** — 부드러운 120 FPS로 최대 2K 해상도를 즐기세요. 나의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비할 데 없는 초저지연 원격 데스크톱 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결하세요. LAN 및 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사 및 붙여넣기하세요. 텍스트, 이미지 및 파일 전송을 기본적으로 완벽하게 지원합니다.

**다중 모니터 지원** — 여러 디스플레이 간에 전환할 수 있는 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 이제 연결 설정에서 직접 어떤 모니터를 볼지 쉽게 선택할 수 있습니다.
...
대부분의 리눅스 원격 데스크톱 에이전트는 Wayland와 호환되지 않거나 세션이 시작될 때마다 권한 프롬프트 및 확인 팝업으로 스팸을 보냅니다.
...
1. **에이전트 설치**: 원격으로 접근하려는 머신에 설치하세요. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근이 필요하면 Tailscale을 연결하세요.

2. **클라이언트 설치**: 작업 공간, 노트북 또는 휴대폰에 설치하세요.
...
소프트웨어 개발 계획 및 향후 기능을 공개 대시보드에서 관리합니다. 현재 개발 중인 사항, 계획된 사항 또는 향후 기능의 상태를 보려면 라이브 로드맵을 확인하세요:
...
우리의 Discord에 참여하여 **베타 테스터** 역할을 받고, 버그를 보고하며, 로드맵을 형성하는 데 도움을 주세요:
