...
**USBridge Remote**는 원격 머신 관리를 위한 통합 고성능 클라이언트입니다. **하드웨어 수준의 BIOS 접근**(USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크탑**을 단일화된 인터페이스로 결합하도록 설계되었습니다.

 🖥️ **OS 부팅 전에 하드웨어 수준의 BIOS 제어가 필요하신가요?**  
 USBridge Remote는 **USBridge-KVM 2.0**과 원활하게 통합되어 금속 수준의 관리가 가능합니다.
...
> Windows Defender는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 감지로 인해 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`). **이는 잘못된 경고입니다.**  
> 우리는 Microsoft 보안 인텔리전스에 파일을 제출하여 공식 검토 및 화이트리스트 등록을 요청했습니다. 그동안 바이러스 백신이 `libva.dll`을 제거하는 경우, 격리에서 복원하거나 USBridge 폴더를 바이러스 백신 제외 목록에 추가해 주세요.
...
### 클라이언트
클라이언트는 제어 인터페이스로, 작업 스테이션이나 노트북에 설치되거나 브라우저에서 직접 실행됩니다. 연결, 실시간 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 관리합니다.
...
**저지연 비디오 및 Moonlight 통합** — 부드러운 240 FPS로 최대 2K 해상도를 즐기세요. 저의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비할 데 없는 초저지연 원격 데스크탑 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결할 수 있습니다. LAN과 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사 및 붙여넣기를 지원합니다. 텍스트, 이미지 및 파일 전송을 기본적으로 완벽하게 지원합니다.

**다중 모니터 지원** — 여러 디스플레이 간 전환 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 연결 설정에서 직접 보고 싶은 모니터를 쉽게 선택할 수 있습니다.
...
대부분의 리눅스 원격 데스크탑 에이전트는 Wayland와 호환되지 않거나 세션이 시작될 때마다 권한 요청 및 확인 팝업으로 스팸을 보냅니다.

USBridge 에이전트는 Wayland를 네이티브로 지원하도록 설계되었습니다. **귀찮은 권한 요청**이나 수동 확인 없이 전체 화면 캡처 및 입력 주입을 기본적으로 처리합니다. 그냥 작동합니다.

---

## 빠른 시작

1. **에이전트 설치**: 원격으로 접근하고자 하는 머신에 설치합니다. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근이 필요하면 Tailscale을 연결하세요.

2. **클라이언트 설치**: 작업 스테이션, 노트북 또는 전화에 설치합니다.
...
소프트웨어 개발 계획 및 향후 기능을 공개 대시보드에서 관리합니다. 현재 개발 중인 사항, 계획된 사항 또는 향후 기능의 상태를 확인하려면 실시간 로드맵을 확인하세요:
