...
**USBridge Remote**는 원격 머신을 관리하기 위한 통합 고성능 클라이언트입니다. **하드웨어 수준 BIOS 접근** (USBridge KVM 장치를 통해)과 **소프트웨어 기반 원격 데스크탑**을 단일화된 인터페이스로 결합하도록 설계되었습니다.
 ...
 USBridge Remote는 **USBridge-KVM 2.0**과 원활하게 통합되어 아웃 오브 밴드, 메탈 수준 관리를 제공합니다.
...
> Windows Defender가 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`) 이는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지 때문입니다. **이는 잘못된 경고입니다.**  
...
### 클라이언트
클라이언트는 제어 인터페이스로, 작업용 컴퓨터나 노트북에 설치되거나 브라우저에서 직접 실행됩니다. 연결, 라이브 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 관리합니다.
...
**모든 것을 위한 하나의 장소** — 워크플로를 통합했습니다. USBridge KVM 하드웨어 및 소프트웨어 에이전트를 단일 대시보드에서 관리하세요. 머신을 추가하고 연결하면 됩니다.
...
**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사 및 붙여넣기가 가능합니다. 텍스트, 이미지 및 파일 전송을 기본적으로 완벽하게 지원합니다.

**다중 모니터 지원** — 여러 디스플레이 간 전환 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 이제 연결 설정에서 쉽게 어떤 모니터를 볼지 선택할 수 있습니다.
...
USBridge 에이전트는 Wayland를 네이티브로 지원하도록 설계되었습니다. 전체 화면 캡처 및 입력 주입을 기본적으로 **귀찮은 권한 프롬프트**나 수동 확인 없이 처리합니다. 그냥 작동합니다.

---

## 빠른 시작

1. **에이전트 설치**: 원격으로 접근하고자 하는 머신에 설치합니다. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근해야 하는 경우 Tailscale을 연결하세요.
...
소프트웨어 개발 계획 및 향후 기능을 공개 대시보드에서 관리합니다. 현재 개발 중인 사항, 계획된 사항 또는 향후 기능의 상태를 확인하고 싶다면 라이브 로드맵을 확인하세요:
...
이 프로젝트는 **GPLv3** 라이센스 하에 제공됩니다 (자세한 내용은 [`LICENSE`](LICENSE)를 참조하세요). Android/Windows/macOS/Linux 클라이언트는 `moonlight-common-c`의 코드를 포함하고 있습니다 (또한 GPLv3).