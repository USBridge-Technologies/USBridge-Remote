...
**USBridge Remote**는 원격 머신을 관리하기 위한 통합 고성능 클라이언트입니다. **하드웨어 수준의 BIOS 접근** (USBridge KVM 장치를 통해)과 **소프트웨어 기반의 원격 데스크탑**을 단일화된 인터페이스로 결합하도록 설계했습니다.

 🖥️ **OS 부팅 전에 하드웨어 수준의 BIOS 제어가 필요하신가요?**  
 USBridge Remote는 **USBridge-KVM 2.0**과 네이티브로 통합되어 아웃 오브 밴드, 메탈 수준의 관리를 제공합니다.
...
> ⚠️ **베타 소프트웨어** — 이는 초기 릴리스입니다. 버그가 발생할 수 있습니다. 문제를 보고하려면 [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues)를 통해 보고하거나 지원을 위해 [Discord](https://discord.com/invite/xqQ6ybkfWS)에 참여해 주세요.
> 
> ℹ️ **Windows Defender / Antivirus 잘못된 경고에 대한 주의:**  
> Windows Defender는 서명되지 않은 바이너리에 대한 휴리스틱/기계 학습 탐지로 인해 `libva.dll`을 위협으로 잘못 표시할 수 있습니다 (`Trojan:Win32/Wacatac.B!ml`). **이는 잘못된 경고입니다.**  
> 우리는 Microsoft Security Intelligence에 파일을 제출하여 공식 검토 및 화이트리스트 등록을 요청했습니다. 그동안, 만약 귀하의 안티바이러스가 `libva.dll`을 제거한다면, 격리에서 복원하거나 USBridge 폴더를 안티바이러스 제외 목록에 추가해 주세요.
...
### 클라이언트
클라이언트는 제어 인터페이스로, 작업용 컴퓨터나 노트북에 설치됩니다. 연결 관리, 라이브 원격 데스크탑, 가상 장치 패스스루 및 스냅샷 레지스트리를 처리합니다.
...
| **ARM64** | — | [다운로드](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest/download/USBridgeClient-macOS-arm64.dmg) | — | [다운로드](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest/download/USBridgeClient-Android-arm64-selfupdate.apk) | [App Store](https://apps.apple.com/us/app/usbridge-client/id6787665935) |
...
| 미디어 | 하이라이트 | 링크 |
| :--- | :--- | :---: |
| **IlSoftware.it** (이탈리아) | *"USBridge Remote는 RustDesk와 AnyDesk에 도전합니다..."* — 소프트웨어 에이전트 및 하드웨어 KVM 시너지, 네이티브 Wayland 지원 및 P2P 아키텍처를 칭찬하는 독립적인 심층 리뷰. | [기사 읽기](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**모든 것을 위한 한 곳** — 워크플로우를 통합했습니다. 단일 대시보드에서 USBridge KVM 하드웨어 및 소프트웨어 에이전트를 관리하세요. 머신을 추가하고 연결하면 됩니다.

**제한 없음, 구독 없음** — 완전히 무료입니다. 세션 시간 제한, 연결 한도 및 대상 머신에서 계정이 필요하지 않습니다.

**저지연 비디오 및 Moonlight 통합** — 부드러운 240 FPS로 최대 2K 해상도를 즐기고 지각할 수 없는 지연이 없습니다. 저의 적응형 스트리밍 엔진은 네이티브 Moonlight 통합을 활용하여 비할 데 없는 초저지연 원격 데스크탑 성능을 제공합니다.

**Tailscale 통합** — 내장된 암호화된 P2P 터널링. 포트 포워딩이나 방화벽 규칙을 건드리지 않고 전 세계의 어떤 머신에도 연결할 수 있습니다. LAN 및 인터넷에서 자동으로 작동합니다.

**공유 클립보드** — 로컬 머신과 원격 대상 간에 원활하게 복사 및 붙여넣기를 지원합니다. 텍스트, 이미지 및 파일 전송을 기본적으로 완벽하게 지원합니다.

**다중 모니터 지원** — 여러 디스플레이 간 전환할 수 있는 기능을 추가했습니다. 대상 머신에 여러 모니터가 있는 경우, 이제 연결 설정에서 쉽게 어떤 모니터를 볼지 선택할 수 있습니다.
...
대부분의 리눅스 원격 데스크탑 에이전트는 Wayland와 호환되지 않거나 세션이 시작될 때마다 권한 프롬프트와 확인 팝업으로 끊임없이 스팸을 보냅니다.

USBridge 에이전트는 Wayland를 네이티브로 지원하도록 설계되었습니다. **귀찮은 권한 프롬프트**나 수동 확인 없이 전체 화면 캡처와 입력 주입을 기본적으로 처리합니다. 그냥 작동합니다.

---

## 빠른 시작

1. **에이전트 설치** — 원격으로 접근하고자 하는 머신에 설치합니다. 실행하면 연결 토큰과 Tailscale 주소가 표시됩니다. 인터넷을 통해 접근해야 하는 경우 Tailscale에 연결합니다.

2. **클라이언트 설치** — 작업용 컴퓨터, 노트북 또는 휴대폰에 설치합니다.
...
소프트웨어 개발 계획 및 향후 기능을 공개 대시보드에서 관리합니다. 현재 개발 중인 것, 계획된 것 또는 향후 기능의 상태를 확인하고 싶다면 라이브 로드맵을 확인하세요:
...
베타 테스터 역할을 얻고 버그를 보고하며 로드맵을 형성하는 데 도움을 주기 위해 Discord에 참여하세요:
