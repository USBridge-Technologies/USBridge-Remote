...
#### Примечание про «как в VirtualBox»

В VirtualBox при абсолютном позиционировании (USB Tablet / mouse integration) **позиция, кнопки и колесо передаются атомарно в одном событии**
(`x/y + wheel + buttonState`). В нашем клиенте и текущем API сервера позиция (`touch_position`) и кнопки/скролл (`click`/`scroll`) — отдельные запросы,
из-за чего теоретически возможна небольшая рассинхронизация.

Чтобы минимизировать эффект, клиент в режиме `absolute` дополнительно **шлёт `touch_position` прямо перед кликом/скроллом**.
