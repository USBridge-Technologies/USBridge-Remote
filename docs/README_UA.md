...
**USBridge Remote** — це єдиний високопродуктивний клієнт для управління віддаленими машинами. Я розробив його, щоб об'єднати **доступ до BIOS на апаратному рівні** (через пристрої USBridge KVM) та **віддалений робочий стіл на програмному рівні** в одному, спрощеному інтерфейсі.
...
> ⚠️ **Бета-програмне забезпечення** — Це ранній реліз. Очікуйте помилки. Будь ласка, повідомляйте про проблеми через [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues) або приєднуйтесь до нашого [Discord](https://discord.com/invite/xqQ6ybkfWS) для підтримки.
> 
> ℹ️ **Примітка щодо помилкових сповіщень Windows Defender / Антивірусів:**  
> Windows Defender може неправильно позначити `libva.dll` як загрозу (`Trojan:Win32/Wacatac.B!ml`) через евристичне/машинне навчання на непідписаних двійкових файлах. **Це помилкове сповіщення.**  
...
### Клієнт
Клієнт — це інтерфейс управління, встановлений на вашій робочій станції або ноутбуці (або запущений безпосередньо у вашому браузері). Він керує з'єднаннями, живим віддаленим робочим столом, пропускною здатністю віртуальних пристроїв і реєстрацією знімків.
...
Вам потрібен прямий APK без облікового запису Play Store? Також опубліковано самовідновлювальну збірку на [останній версії](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest).

🌐 **Веб-клієнт без установки**: Установка не потрібна. Просто відкрийте [web.usbridge.io](https://web.usbridge.io), щоб миттєво підключитися. *(Примітка: веб-клієнт працює з деякими обмеженнями функцій і продуктивності через безпеку браузера та обмеження WebRTC. Для повного досвіду без компромісів використовуйте рідні додатки).*

## Агент

Агент працює на цільовій машині — сервері або ПК, до якого ви хочете отримати віддалений доступ. Він обробляє захоплення екрана, введення даних та мережі Tailscale.
...
**Одне місце для всього** — Я об'єднав робочий процес. Керуйте апаратним забезпеченням USBridge KVM та програмними агентами з одного інформаційного панелі. Додайте машину, підключіться, і ви всередині.

**Без обмежень, без підписок** — Повністю безкоштовно. Немає обмежень на час сесії, немає обмежень на з'єднання та не потрібен обліковий запис на цільовій машині.

**Відео з низькою затримкою та інтеграція Moonlight** — Насолоджуйтесь роздільною здатністю до 2K з плавними 120 FPS та нульовою помітною затримкою. Мій адаптивний стрімінговий двигун використовує нативну інтеграцію Moonlight для забезпечення безпрецедентної продуктивності віддаленого робочого столу з ультра-низькою затримкою.

**Інтеграція Tailscale** — Вбудоване зашифроване P2P тунелювання. Підключайтеся до будь-якої машини в усьому світі без налаштування переадресації портів або правил брандмауера. Це працює в локальній мережі та через Інтернет автоматично.

**Спільний буфер обміну** — Копіюйте та вставляйте безперешкодно між вашою локальною машиною та віддаленими об'єктами. Він повністю підтримує текст, зображення та передачу файлів з коробки.
...
Я розробив USBridge Agent для нативної підтримки Wayland. Він обробляє повне захоплення екрана та введення даних з коробки **без будь-яких набридливих запитів на дозвіл** або ручних підтверджень. Він просто працює.
...
Цей проект ліцензовано під **GPLv3** (див. [`LICENSE`](LICENSE)). Клієнт для Android/Windows/macOS/Linux включає код з `moonlight-common-c` (також GPLv3).