...
**USBridge Remote** — це єдиний високопродуктивний клієнт для управління віддаленими машинами. Я розробив його, щоб поєднати **доступ до BIOS на апаратному рівні** (через пристрої USBridge KVM) та **віддалений робочий стіл на програмному рівні** в одному, спрощеному інтерфейсі.
...
> ⚠️ **Бета-програмне забезпечення** — Це ранній реліз. Очікуйте на помилки. Будь ласка, повідомляйте про проблеми через [GitHub Issues](https://github.com/USBridge-Technologies/USBridge-Remote/issues) або приєднуйтесь до нашого [Discord](https://discord.com/invite/xqQ6ybkfWS) для підтримки.
> 
> ℹ️ **Примітка щодо хибних сповіщень Windows Defender / Антивірусів:**  
> Windows Defender може неправильно позначити `libva.dll` як загрозу (`Trojan:Win32/Wacatac.B!ml`) через евристичне/машинне навчання на неподписаних двійкових файлах. **Це хибне сповіщення.**  
> Ми надіслали файл Microsoft Security Intelligence для офіційного перегляду та внесення до білого списку. Тим часом, якщо ваш антивірус видалить `libva.dll`, будь ласка, відновіть його з карантину або додайте папку USBridge до списку виключень вашого антивірусу.
...
### Клієнт
Клієнт — це інтерфейс управління, встановлений на вашій робочій станції або ноутбуці (або запущений безпосередньо у вашому браузері). Він керує з'єднаннями, живим віддаленим робочим столом, пропуском віртуальних пристроїв та реєстром знімків.
...
Вам потрібен прямий APK без облікового запису Play Store? Також опубліковано самовідновлювальну версію на [останній реліз](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest).

🌐 **Веб-клієнт без установки**: Установка не потрібна. Просто відкрийте [web.usbridge.io](https://web.usbridge.io), щоб підключитися миттєво. *(Примітка: Веб-клієнт працює з деякими обмеженнями функцій та продуктивності через безпеку браузера та обмеження WebRTC. Для повного досвіду без компромісів використовуйте нативні додатки).*
...
| ЗМІ | Основна інформація | Посилання |
| :--- | :--- | :---: |
| **IlSoftware.it** (Італія) | *"USBridge Remote кидає виклик RustDesk та AnyDesk..."* — незалежний огляд, що хвалить синергію програмного агента та апаратного KVM, нативну підтримку Wayland та P2P архітектуру. | [Читати статтю](https://www.ilsoftware.it/alternativa-rustdesk-anydesk-usbridge/) |
...
**Одне місце для всього** — Я об'єднав робочий процес. Керуйте апаратним забезпеченням USBridge KVM та програмними агентами з єдиної панелі управління. Додайте машину, підключіться, і ви в справі.
...
**Відео з низькою затримкою та інтеграція Moonlight** — Насолоджуйтесь роздільною здатністю до 2K з плавними 240 FPS та нульовою затримкою. Мій адаптивний стрімінговий двигун використовує нативну інтеграцію Moonlight для забезпечення безпрецедентної продуктивності віддаленого робочого столу з наднизькою затримкою.

**Інтеграція Tailscale** — Вбудоване зашифроване P2P тунелювання. Підключайтеся до будь-якої машини в усьому світі без необхідності налаштування переадресації портів або правил брандмауера. Це працює в локальній мережі та через інтернет автоматично.
...
Я розробив USBridge Agent для нативної підтримки Wayland. Він обробляє повне захоплення екрана та введення даних з коробки **без жодних дратівливих запитів на дозвіл** або ручних підтверджень. Він просто працює.
...
Я управляю планами розробки програмного забезпечення та майбутніми функціями в відкритій панелі управління. Якщо ви хочете побачити, що наразі розробляється, що заплановано, або відстежувати статус майбутніх функцій, перегляньте живу дорожню карту:
...
- 🌐 [Офіційний веб-сайт](https://usbridge.io)
...
Цей проект ліцензовано під **GPLv3** (див. [`LICENSE`](LICENSE)). Клієнт для Android/Windows/macOS/Linux містить код з `moonlight-common-c` (також GPLv3).