...
**USBridge Remote** — це єдиний високопродуктивний клієнт для управління віддаленими машинами. Я розробив його, щоб поєднати **доступ до BIOS на апаратному рівні** (через пристрої USBridge KVM) та **десктоп віддаленого доступу на програмному рівні** в одному, спрощеному інтерфейсі.
...
> 
🔥 **Створіть свій власний IP-KVM!**
Я офіційно випустив прошивку для DIY. Тепер ви можете перетворити свій сумісний SBC (такий як Radxa Zero 3W/3E або Cubie A7) та USB-кадр захоплення на повноцінний пристрій USBridge KVM 2.0.
...
### Клієнт
Клієнт — це інтерфейс управління, встановлений на вашій робочій станції або ноутбуці (або запускається безпосередньо у вашому браузері). Він керує з'єднаннями, живим віддаленим робочим столом, пропуском віртуальних пристроїв та реєстрацією знімків.
...
Вам потрібен прямий APK без облікового запису Play Store? Автоматично оновлювана версія також опублікована на [останній версії](https://github.com/USBridge-Technologies/USBridge-Remote/releases/latest).

🌐 **Веб-клієнт без установки**: Установка не потрібна. Просто відкрийте [web.usbridge.io](https://web.usbridge.io) для миттєвого підключення. *(Примітка: Веб-клієнт працює з деякими обмеженнями функцій та продуктивності через безпеку браузера та обмеження WebRTC. Для повноцінного досвіду використовуйте нативні додатки).*

## Агент

Агент працює на цільовій машині — сервері або ПК, до якого ви хочете отримати віддалений доступ. Він обробляє захоплення екрана, ін'єкцію вводу та мережі Tailscale.
...
**Без обмежень, без підписок** — Абсолютно безкоштовно. Немає обмежень за часом сесії, немає обмежень на підключення і не потрібно облікового запису на цільовій машині.

**Відео з низькою затримкою та інтеграція Moonlight** — Насолоджуйтесь роздільною здатністю до 2K з плавними 120 FPS і нульовою затримкою. Мій адаптивний стрімінговий движок використовує нативну інтеграцію Moonlight для забезпечення безпрецедентної продуктивності віддаленого робочого столу з ультра-низькою затримкою.
...
**Підтримка кількох моніторів** — Я додав можливість перемикатися між кількома дисплеями. Якщо цільова машина має кілька моніторів, ви тепер можете легко вибрати, який з них переглядати безпосередньо з налаштувань підключення. 
...
Я розробив USBridge Agent для нативної підтримки Wayland. Він обробляє повне захоплення екрана та ін'єкцію вводу з коробки **без жодних дратівливих запитів на дозвіл** або ручних підтверджень. Він просто працює.

---

## Швидкий старт

1. **Встановіть Агент** на машині, до якої ви хочете отримати віддалений доступ. Запустіть його — він відобразить токен підключення та адресу Tailscale. Підключіть Tailscale, якщо вам потрібен доступ через Інтернет.
