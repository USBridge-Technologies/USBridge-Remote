...
> Windows Defender може неправильно позначити `libva.dll` як загрозу (`Trojan:Win32/Wacatac.B!ml`) через евристичне/машинне навчання для виявлення незахищених бінарних файлів. **Це помилкове сповіщення.**  
...
### Клієнт
Клієнт — це інтерфейс управління, встановлений на вашій робочій станції або ноутбуці. Він управляє з'єднаннями, живим віддаленим робочим столом, передачею віртуальних пристроїв та реєстрацією знімків.
...
Агент працює на цільовій машині — сервері або ПК, до якого ви хочете отримати віддалений доступ. Він обробляє захоплення екрану, ін'єкцію введення та мережеву взаємодію Tailscale.
...
**Одне місце для всього** — Я об'єднав робочий процес. Керуйте апаратним забезпеченням USBridge KVM та програмними агентами з єдиної панелі управління. Додайте машину, підключіться, і ви готові.

**Без обмежень, без підписок** — Абсолютно безкоштовно. Без обмежень за часом сесії, без обмежень на з'єднання і без необхідності створення облікового запису на цільовій машині.

**Відео з низькою затримкою та інтеграція Moonlight** — Насолоджуйтесь роздільною здатністю до 2K з плавними 240 FPS і нульовою затримкою. Мій адаптивний стрімінговий двигун використовує нативну інтеграцію Moonlight для забезпечення безпрецедентної продуктивності віддаленого робочого столу з ультра-низькою затримкою.
...
Я розробив USBridge Agent для нативної підтримки Wayland. Він обробляє повне захоплення екрану та ін'єкцію введення з коробки **без будь-яких дратівливих запитів на дозвіл** або ручних підтверджень. Він просто працює.
...
Я управляю планами розробки програмного забезпечення та майбутніми функціями в відкритій панелі управління. Якщо ви хочете побачити, що наразі розробляється, що заплановано, або слідкувати за статусом майбутніх функцій, перегляньте живу дорожню карту:
